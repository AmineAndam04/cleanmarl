...
class RolloutBuffer:
    ...
    def get_batch(self):
        ...
        avail_actions = (
            torch.ones(self.buffer_size, max_length, self.num_agents, self.action_space)
            .bool()
            .to(self.device)
        )
        actions = torch.zeros(self.buffer_size, max_length, self.num_agents).long().to(self.device)
        rewards = torch.zeros(self.buffer_size, max_length).float().to(self.device)
        returns = torch.zeros(self.buffer_size, max_length, self.num_agents).float().to(self.device)
        states = torch.zeros(self.buffer_size, max_length, self.state_space).float().to(self.device)
        mask = torch.zeros(self.buffer_size, max_length).bool().to(self.device)
...
if __name__ == "__main__":
    ...
    if args.normalize_obs:
        envs = NormalizeVecObservation(envs, normalize_state=True)
    ...
    critic_input_dim = get_coma_critic_input_dim(envs)
    critic = Critic(
    ...
    log_dir = f"{args.work_dir}/COMA-lstm-multienvs-{run_name}"
    ...
    while step < args.total_timesteps:
        ...
        while envs.get_env_mask().any():
            ...
            with torch.no_grad():
                ...
                actions, h = actor.act(
                    ...
                    h=h,
                    eps=epsilon,
                    avail_action=torch.from_numpy(avail_action).bool().flatten(0, 1).to(device),
                )
                actions = actions.reshape(args.batch_size, envs.n_agents)
                # Compute its value
        ...
        # ---- Training loop -------
        # Prepare the batch
        b_obs, b_actions, b_rewards, b_returns, b_states, b_avail_actions, b_mask = rb.get_batch()
        # Update critic and actor
        num_samples = b_mask.sum() * envs.n_agents
        ...
        for start in range(0, b_obs.size(1), args.tbptt):
            ...
            mb_obs, mb_actions, mb_rewards, mb_returns, mb_states, mb_avail_actions, mb_mask = (
                b_obs[:, start:end],
                b_actions[:, start:end].flatten(0, 1),
                b_rewards[:, start:end],
                b_returns[:, start:end].flatten(0, 1),
                b_states[:, start:end].flatten(0, 1),
                b_avail_actions[:, start:end],
                b_mask[:, start:end].flatten(0, 1),
            )
            # Critic loss
            b_q_values = critic(state=mb_states, observations=mb_obs.flatten(0, 1), actions=mb_actions)
            cr_values = torch.gather(b_q_values, dim=-1, index=mb_actions.unsqueeze(-1))
            critic_loss = F.mse_loss(
                cr_values[mb_mask].reshape(-1), mb_returns[mb_mask].reshape(-1), reduction="sum"
            )
            ...
            pi, h = actor.logits(
                ...
                h=h,
                avail_action=mb_avail_actions.permute(0, 2, 1, 3).flatten(0, 1),
            )
            pi = pi.reshape(args.batch_size, envs.n_agents, mb_obs.size(1), -1).transpose(1, 2).flatten(0, 1)
            log_pi = torch.log(pi + 1e-8)
            entropy_loss = -(pi * log_pi).sum(-1)
            ...
            current_q = torch.gather(ac_values, dim=-1, index=mb_actions.unsqueeze(-1)).squeeze(-1)
            advantage = (current_q - coma_baseline).detach()
            if args.normalize_advantage:
                advantage = (advantage - advantage[mb_mask].mean()) / (advantage[mb_mask].std() + 1e-8)
            log_pi = torch.gather(log_pi, dim=-1, index=mb_actions.unsqueeze(-1)).squeeze(-1)
            actor_loss = (log_pi * advantage)[mb_mask].sum()
