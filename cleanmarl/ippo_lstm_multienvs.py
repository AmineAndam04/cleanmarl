...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(f"{args.work_dir}/IPPO-lstm-multienvs-{run_name}")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n{}".format(
    ...
    while step < args.total_timesteps:
        ...
        with torch.no_grad():
            for ep_idx in range(return_lambda.size(0)):
                ...
                next_value = torch.cat((next_value, torch.zeros((1, eval_env.n_agents), device=device)))
        ...
        b_mask = b_mask.unsqueeze(-1).expand(-1, -1, eval_env.n_agents).permute(0, 2, 1).flatten(0, 1)
