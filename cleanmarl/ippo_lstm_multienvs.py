    ...
    while step < args.total_timesteps:
        ...
        while len(alive_envs) > 0:
            with torch.no_grad():
                ...
                actions,log_probs,alive_h = actor.act(torch.from_numpy(obs).float().to(device),
                                                      h=alive_h,
        ...
        for _ in range(args.epochs):
            ...
            for t in range(b_obs.size(1)):
                ...
                current_logits = current_logits.reshape(args.batch_size,eval_env.n_agents,-1)
        ...
        if (training_step/args.epochs) % args.eval_steps == 0:
            ...
            while eval_ep < args.num_eval_ep:
                with torch.no_grad():
                    actions,_,h_eval = actor.act(torch.from_numpy(eval_obs).float().to(device),
                                                 h=h_eval,
