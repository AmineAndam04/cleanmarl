...
class Args:
    ...
    batch_size: int = 64
    """ Batch size to train MLP value function"""
    optimizer: str = "Adam"
    """ The optimizer"""
...
class RolloutBuffer:
    ...
    def get_batch(self):
        ...
        advantages = torch.zeros(self.buffer_size, max_length).float().to(self.device)
        ...
        advantages = advantages.unsqueeze(-1).expand(-1, -1, self.num_agents)
        mask = mask.unsqueeze(-1).expand(-1, -1, self.num_agents)
        ...
        return (
            ...
            returns,
            states,
            mask.permute(0, 2, 1).flatten(0, 1),
        )
...
if __name__ == "__main__":
    ...
    log_dir = f"{args.work_dir}/MAPPO-lstm-{run_name}"
