...
if __name__ == "__main__":
    ...
    agents_target_network = Qnetwrok(input_dim=env.get_obs_size(),
                                          ...
                                          num_layer=args.num_layers,
                                          output_dim=env.get_action_size())
    mixing_network = MixingNetwork(n_agents=env.n_agents,
                                    s_dim=env.get_state_size(),
                                    hidden_dim=args.hyper_dim,
                                   )
    target_mixing_network = MixingNetwork(n_agents=env.n_agents,
                                    s_dim=env.get_state_size(),
                                    hidden_dim=args.hyper_dim,
                                   )
    soft_update(
    ...
    # episode = {"obs": [],"actions":[],"reward":[],"next_obs":[],"states":[],"done":[],"avail_actions":[]}
    # obs,_ = env.reset()
    ...
    while step < args.total_timesteps:
        ...
        while not done and not truncated:
            ...
            avail_action = torch.tensor(env.get_avail_actions(), dtype=torch.bool, device=args.device) # Get the mask of 'next_obs' and store it in the replay, we need it for the bellman loss
    ...
    #     if num_episode > args.batch_size:
    #         if num_episode % args.train_freq == 0:
    #             batch_obs,batch_action,batch_reward,batch_next_obs,batch_states,batch_avail_action,batch_done, batch_mask = rb.sample(args.batch_size)
    #             loss = 0
    #             for t in range(batch_obs.size(1)):
    #                 with torch.no_grad():
    #                     q_next_max,_ = agents_target_network(batch_next_obs[:,t],avail_action =batch_avail_action[:,t] ).max(dim=-1)
    #                 q_tot_target = q_next_max.sum(dim=-1)
    #                 targets = batch_reward[:,t] + args.gamma * (1-batch_done[:,t])*q_tot_target #(torch.sum(q_next_max, dim=-1))
    ...
    #                 print(batch_mask[:,t])
    #                 loss += F.mse_loss(targets[batch_mask[:,t]],q_tot[batch_mask[:,t]])
    #             optimizer.zero_grad()
    #             loss.backward()
    #             grads = [p.grad for p in agents_utility_network.parameters() ]
    #             grad_norm_2 = norm_d(grads,2)
