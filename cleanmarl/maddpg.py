...
class Actor(nn.Module):
    ...
    def act(self,x,avail_action=None,hard=False,printit= False):
        x = self.logits(x,avail_action,printit)
        actions = F.gumbel_softmax(logits=x,hard=hard)
        return actions
    def logits(self,x,avail_action=None,printit=False):
...
class Critic(nn.Module):
    ...
    def forward(self,state,actions,grad_processing=False):
        
        x = self.maddpg_inputs(state,actions,grad_processing)
        for layer in self.layers:
            x = layer(x)
        return x.squeeze()
    def maddpg_inputs(self,state ,actions,grad_processing):
        ...
        if grad_processing:
            ...
            oh = torch.where(mask.bool(),oh, oh.clone().detach())
        coma_inputs[:,:,state.size(-1):] = oh
        return coma_inputs
...
if __name__ == "__main__":
    ...
    coma_input_dim = env.get_state_size() +  env.n_agents*env.get_action_size()
    critic = Critic(
        input_dim=coma_input_dim,
        ...
        if num_episode > args.batch_size:
            ...
            if num_episode % args.train_freq == 0:
                ...
                for t in range(batch_obs.size(1)-1):
                    ...
                    qvals = critic(batch_states[:,t],actions,grad_processing=True)
