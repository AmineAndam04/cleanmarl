...
if __name__ == "__main__":
        ...
        if num_episode > args.batch_size:
            if num_episode % args.train_freq == 0:
                ...
                for t in range(0, batch.batch_obs.size(1), args.minibatch_size):
                    ...
                    critic_loss += (
                        F.mse_loss(targets[mb_mask], q_values[mb_mask]) * mb_mask.sum()
                    )
                ...
                for t in range(0, batch.batch_obs.size(1), args.minibatch_size):
                    ...
                    actions = actor.act(
                        mb_obs, avail_action=mb_avail_action, hard=False
                    )
                    qvals = critic(
                        mb_states,
                        actions,
    ...
    if args.save_model:
        # Save the weights
        actor_model_path = f"{args.work_dir}/MADDPG-{run_name}/agent.pt"
